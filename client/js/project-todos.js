...
function getProjectTodos(id) {
  ...
  axios({
    ...
    `)
      ...
      console.log(data)
      console.log(todos, '<<<<');
      $('.asido').append(`<div class="mb-3"><h3>Members</h3></div><div><h5><i class="fas fa-crown"></i> ${project.owner.name}</h5></div>`)
      if (localStorage.getItem('_id') === project.owner._id) {
        members.forEach(el => {
          let html = `<div class="kick" onclick="removeMember('${el._id}')"><h5><i class="far fa-times-circle"></i> ${el.name}</h5></div>`
          $('.asido').append(html)
        })
      }
      else {
        members.forEach(el => {
          let html = `<div><h5><i class="fas fa-user-alt"></i> ${el.name}</h5></div>`
          $('.asido').append(html)
        })
      }
